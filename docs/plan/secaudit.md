...
| ~~`trivy`~~ | **removed 2026-09-08** | Image CVE scanning moved to the application's CI (docs/CI-IMAGE-SCANNING.md). The fix — a base-image tag or a lockfile — lives in a repository, so the gate belongs at the build, where it arrives with the change that caused it and in front of the person who can act. The fleet kept the question CI cannot answer, *is what is running what we shipped*, as `secaudit_running_image_age_seconds` from metadata Komodo already reports. This removed ~27% of `run.py` and 1.3 GB per host, and with them the memory floor/ceiling, the step budget, the rotation cursor, the `in_use` matching and the Java-DB trap — nearly every bug phase 2 hit was in that one step. |
